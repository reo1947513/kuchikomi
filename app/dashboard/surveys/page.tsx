...
export default function DashboardPage() {
  ...
  const [lineUserId, setLineUserId] = useState<string | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me").then(r => r.json()).then(d => setLineUserId(d.lineUserId ?? null)).catch(() => setLineUserId(null));
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {lineUserId !== undefined && (
        <div className={`rounded-xl border p-3 flex items-center justify-between ${lineUserId ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-200"}`}>
          <div className="flex items-center gap-3">
            <svg className="w-6 h-6" viewBox="0 0 24 24" fill={lineUserId ? "#06C755" : "#9CA3AF"}>
            ...
            <div>
              <p className={`text-sm font-medium ${lineUserId ? "text-green-700" : "text-gray-600"}`}>
                {lineUserId ? "LINE連携済み" : "LINE未連携"}
              </p>
              <p className="text-xs text-gray-400">
                {lineUserId ? "レビュー通知・月次レポートがLINEに届きます" : "連携するとレビュー通知をLINEで受け取れます"}
              </p>
            </div>
          </div>
          {!lineUserId && (
            <a href="/api/line/auth" className="px-4 py-2 bg-[#06C755] hover:bg-[#05b04c] text-white text-sm font-semibold rounded-lg transition-colors">
              連携する
            </a>
          )}
        </div>
