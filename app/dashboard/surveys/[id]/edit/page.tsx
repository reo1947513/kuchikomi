      ...
      setQuestions(
        (data.questions ?? []).map(
          (q: {
            ...
            order: number;
            type: string;
            choices?: { text: string; order: number; score: number }[];
          }) => ({
            text: q.text,
            order: q.order,
            type: q.type === "text" ? "text" : "choice",
            choices: (q.choices ?? []).map(
              (c: { text: string; order: number; score: number }) => ({
                text: c.text,
                order: c.order,
                score: c.score,
              })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!questionsRes.ok) {
        ...
        setError(data.error ?? "質問の更新に失敗しました");
        return;
      }
          ...
          <div className="space-y-4">
            ...
            {questions.map((question, qIdx) => (
              <QuestionBlock
                ...
                onUpdateChoice={(cIdx, patch) =>
                  updateChoice(qIdx, cIdx, patch)
                }
              />
            ))}
