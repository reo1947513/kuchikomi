...
const CATEGORIES = ["サービスについて", "料金について", "解約について", "その他"];
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              {touched.category && category && <div className="absolute right-3 top-1/2 -translate-y-1/2 sm:right-[calc(100%-17rem)]"><svg className="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>}
            </div>
          </div>
          <div>
            <label className="block text-sm font-bold text-gray-800 mb-1">ご契約状況<RequiredBadge /></label>
            <div className="flex gap-4">
              {["契約中", "未契約"].map((s) => (
                <label key={s} className={`flex-1 flex items-center justify-center gap-2 border rounded-lg px-4 py-2.5 cursor-pointer transition-colors ${contractStatus === s ? "border-green-500 ring-1 ring-green-500" : "border-gray-300"}`}>
                  <input type="radio" name="contract" value={s} checked={contractStatus === s} onChange={() => setContractStatus(s)} className="text-violet-500 focus:ring-violet-400" />
